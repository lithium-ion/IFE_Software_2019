...
int numBoards = 12;
int cellsPerBoard = 8;
bool cellsDischarge[12][8];			// true for discharging, false for not discharging
	...
	for (int board = 0; board < numBoards; board++) {
		for (int cell = 0; cell < cellsPerBoard; cell++) {
			...
			if (voltage > (lowestVoltage + balancingDifference)) {
				cellsDischarge[board][cell] = true;
			}
